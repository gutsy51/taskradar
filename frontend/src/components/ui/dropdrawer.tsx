...
// Custom checkbox component with color variants
  ...
  const childrenWithSeparators = React.useMemo(() => {
    ...
    return filteredChildren.flatMap((child, index) => {
      ...
      return [
        child,
        <div
          // biome-ignore lint/suspicious/noArrayIndexKey: acceptable for separator
          key={`separator-${index}`}
...
function DropDrawerSubTrigger({
  ...
  const checkDataAttrubutes = (element: HTMLElement) => {
  ...
  if (isMobile) {
    ...
    const handleClick = (e: React.MouseEvent) => {
      ...
      submenuId = checkDataAttrubutes(element);
  ...
  if (isMobile) {
    ...
    return (
      <button
        ...
        className={cn(
          "flex cursor-pointer items-center justify-between px-4 py-4 w-full text-left",
          "bg-accent dark:bg-accent px-2 my-1.5 rounded-md",
          disabled && "pointer-events-none opacity-50",
          className,
        )}
        onClick={handleClick}
